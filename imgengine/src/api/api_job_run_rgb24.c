...
static img_result_t img_api_validate_rgb24_input(
    const uint8_t *input,
    size_t input_size,
    uint32_t width,
    uint32_t height,
    ...
    const img_job_t *job)
{
    ...
    img_result_t sec = img_api_validate_rgb24_input(
        input, input_size, width, height, &stride);
    if (sec != IMG_SUCCESS)
        return sec;
    ...
    size_t *output_size)
{
    ...
    r = img_api_render_job_output(
        &ctx, &canvas, &layout, job, output, output_size);
    img_api_finish_job_stage(engine, &canvas, arena);
    return r;
}
