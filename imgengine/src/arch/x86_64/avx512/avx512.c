...
static inline void img_resize_blend16_rgb(
    uint8_t *dst_row,
    const uint8_t *row0,
    const uint8_t *row1,
    const uint32_t *x_index,
    const uint16_t *x_weight,
    uint32_t x_ratio,
    uint32_t src_w,
    uint32_t base_dx,
    uint32_t fy,
    uint32_t wy0)
{
    ...
    const __m512i wx0_v = _mm512_setr_epi32(
        (int32_t)wx0[0], (int32_t)wx0[1], (int32_t)wx0[2], (int32_t)wx0[3],
    ...
    const __m512i fx_v = _mm512_setr_epi32(
        (int32_t)fx[0], (int32_t)fx[1], (int32_t)fx[2], (int32_t)fx[3],
    ...
    for (uint32_t c = 0; c < 3; c++)
    {
        ...
        for (uint32_t lane = 0; lane < 16; lane++)
        {
            dst_row[(size_t)(base_dx + lane) * 3u + c] =
                img_resize_clamp_u8(out_i[lane]);
        }
    }
}
...
void img_arch_resize_avx512(img_ctx_t *ctx, img_buffer_t *dst, void *params)
{
    ...
    for (uint32_t dy = 0; dy < dst_h; dy++)
    {
        ...
        {
            img_resize_blend16_rgb(
                dst_row, row0, row1, p->x_index, p->x_weight, x_ratio, src_w, dx, fy, wy0);
        }
